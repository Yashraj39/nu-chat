...
import { deleteMessage, fileContentUrl, fileDownloadUrl, klipyMediaUrl, messages, sendFile, sendText, upload } from "../api";
...
export function ChatPage({ user, incomingMessage, connected, typingUsers, onTyping }: ChatPageProps) {
    ...
    useEffect(() => () => { if (typingTimerRef.current) window.clearTimeout(typingTimerRef.current); if (typingRef.current) onTyping(false); }, [onTyping]);
    ...
    function handleMediaLoaded(){if(!stickToBottomRef.current)return;requestAnimationFrame(()=>{const c=list.current;if(c)c.scrollTop=c.scrollHeight;});}
    function handleScroll(){const c=list.current;if(!c||initializingRef.current)return;const d=c.scrollHeight-c.scrollTop-c.clientHeight;const at=d<100;stickToBottomRef.current=at;setNearBottom(at);}
    function replyTo(m:Message){setReplyingTo(m);requestAnimationFrame(()=>document.getElementById("chat-composer")?.focus());}
    function jumpToMessage(id:string){const e=messageRefs.current[id];if(!e)return;e.scrollIntoView({behavior:"smooth",block:"center"});e.classList.add("ring-2","ring-indigo-400","rounded-xl");setTimeout(()=>e.classList.remove("ring-2","ring-indigo-400","rounded-xl"),1200);}
    ...
    return <main className="page"><section className="chat-shell"><div className="chat-head"><div><h1 className="font-bold text-lg">Campus Group</h1><p className="muted text-xs">Everyone can chat here</p></div><div className="flex items-center gap-2"><div className="flex items-center gap-2 rounded-full border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 px-3 py-1.5" title="Your display name"><span className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">You</span><span className="max-w-[180px] truncate text-xs font-bold text-slate-700 dark:text-slate-200">{user.displayName}</span></div><div className={`status ${connected?"online":"offline"}`}>{connected?<Wifi size={14}/>:<WifiOff size={14}/>} {connected?"Connected":"Reconnecting…"}</div></div></div>
        ...
        {typingLabel&&<div className="typing-indicator-wrap" aria-live="polite"><div className="typing-indicator"><span className="typing-avatar">{typingLabel.charAt(0).toUpperCase()}</span><span className="typing-label">{typingLabel}</span><span className="typing-dots" aria-hidden="true"><i/><i/><i/></span></div></div>}
        ...
        {replyingTo&&<ReplyComposerPreview message={replyingTo} onCancel={cancelReply} onJump={jumpToMessage}/>}<div className="composer"><label className="iconbtn cursor-pointer" title={replyingTo?"Attach file as reply":"Attach any file"}><Paperclip size={20}/><input hidden type="file" onChange={attach}/></label><button type="button" className="iconbtn" title="GIFs & Stickers" onClick={()=>setShowMediaPicker(true)}><SmilePlus size={20}/></button><textarea id="chat-composer" value={text} onChange={e=>setTypingState(e.target.value)} onBlur={stopTyping} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();submit();}if(e.key==="Escape"&&replyingTo){cancelReply();stopTyping();}}} placeholder={replyingTo?"Reply to message…":"Type a message…"} rows={1}/><button className="btn-primary send" disabled={!connected||!text.trim()} onClick={submit}><Send size={18}/></button></div>{showMediaPicker&&<KlipyPicker replyToMessageId={replyingTo?.id} onClose={()=>setShowMediaPicker(false)}/>}</section></main>;
}
...
function MediaBubble({media,type,messageId,onLoaded}:{media:Message["media"];type:"GIF"|"STICKER";messageId:string;onLoaded:()=>void}){
    ...
    return <div className={type==="STICKER"?"flex justify-center":"max-w-sm"}><img src={src} alt={media?.title||type} loading="lazy" onLoad={onLoaded} onError={()=>{if(!fallback&&fallbackUrl!==proxyUrl)setFallback(true);else console.warn("Media failed to load",messageId);}} className={type==="STICKER"?"max-w-[180px] max-h-[180px] object-contain":"max-w-full rounded-lg"}/></div>;
}
